...
const toEntityConnectivityStat = (stat: Record<string, any> | null | undefined) => {
  if (!stat) return null;

  return {
    benchmark_metadata: stat.benchmark_metadata,
    countries_with_realtime_data: stat.countries_with_realtime_data ?? 0,
    entity_with_realtime_data: stat.entity_with_realtime_data ?? stat.school_with_realtime_data ?? 0,
    ...
    no_of_entities_measure: stat.no_of_entities_measure ?? stat.no_of_schools_measure ?? 0,
    real_time_connected_entities: stat.real_time_connected_entities ?? stat.real_time_connected_schools ?? {},
  };
};

const hasConnectivityLayerInfo = (stat: Record<string, any> | null | undefined) => {
  return !!stat && ['live_avg', 'no_of_entities_measure', 'no_of_schools_measure', 'real_time_connected_entities', 'real_time_connected_schools'].some((key) => key in stat);
};

$connectivityStatsByEntity.on(fetchEntitiesLayerInfoFx.doneData, (state, payload) => {
  if (!Object.values(payload ?? {}).some(hasConnectivityLayerInfo)) return state;
...
const toConnectivityStat = (stat: Record<string, any> | null | undefined): ConnectivityStat | null => {
  if (!stat) return null;
  ...
  return {
    ...
    no_of_schools_measure: stat.no_of_schools_measure ?? stat.no_of_entities_measure ?? 0,
    real_time_connected_schools: stat.real_time_connected_schools ?? stat.real_time_connected_entities ?? {},
    school_with_realtime_data: stat.school_with_realtime_data ?? stat.entity_with_realtime_data ?? 0,
  };
};
...
export const $coverageStatsByEntity = createStore<Partial<Record<EntityType, CoverageStat>>>({});

const toCoverageStat = (stat: Record<string, any> | null | undefined): CoverageStat | null => {
  if (!stat) return null;
...
const hasCoverageLayerInfo = (stat: Record<string, any> | null | undefined) => {
  return !!stat && ['connected_schools', 'connected_entities', 'total_schools', 'total_entities'].some((key) => key in stat);
};

$coverageStatsByEntity.on(fetchEntitiesLayerInfoFx.doneData, (state, payload) => {
...
export const onSelectEntityStatusLayer = createEvent<{ entityType: EntityType; layerId: number | null }>();
export const $statusLayerIdByEntity = createStore<EntityStoreMap<number | null>>(defaultEntityStatusLayerSelection);
$statusLayerIdByEntity.on(onSelectSchoolStatusLayer, (state, layerId) => ({
  ...state,
  [$selectedEntityType.getState()]: layerId,
}));
$statusLayerIdByEntity.on(onSelectEntityStatusLayer, (state, { entityType, layerId }) => ({
  ...state,
  [entityType]: layerId,
}));
export const $schoolStatusSelectedLayer = combine(
  ...
  $selectedEntityType,
  (statusLayerIdByEntity, selectedEntityType) => getEntityValue(statusLayerIdByEntity, selectedEntityType, SCHOOL_STATUS_LAYER.id)
);

export const onSelectMainLayer = createEvent<number | null>();
export const onSelectEntityMainLayer = createEvent<{ entityType: EntityType; layerId: number | null }>();
export const $selectedLayerId = restore(onSelectMainLayer, null);
$selectedLayerId.on(onSelectEntityMainLayer, (_, { layerId }) => layerId);
export const $selectedLayerIdByEntity = createStore<EntityStoreMap<number | null>>({});
$selectedLayerIdByEntity.on(onSelectMainLayer, (state, layerId) => ({
  ...state,
  [$selectedEntityType.getState()]: layerId,
}));
$selectedLayerIdByEntity.on(onSelectEntityMainLayer, (state, { entityType, layerId }) => ({
  ...state,
  [entityType]: layerId,
}));
export const $globalLayerData = $layersList.map(layers => layers?.find(layer => layer?.type === LayerTypeChoices.LIVE && !layer.created_by && (layer as LayerType & { code?: string }).code === 'DOWNLOAD') ?? null);
...
export const $activeLayerByCountriesByEntity = combine($layersList, $countryIdToCode, $activeEntityTypes, (layers, countryIdToCode, activeEntityTypes) => {
...
export const $activeLayerByCountries = combine($activeLayerByCountriesByEntity, $selectedEntityType, (activeLayersByEntity, selectedEntityType) => {
  return getEntityValue(activeLayersByEntity, selectedEntityType, { list: {}, countryDefaultLayerList: {} });
})

export const $currentDefaultLayerIdByEntity = combine($countryCode, $activeLayerByCountriesByEntity, $globalLayerId, (countryCode, activeLayersByEntity, globalLayerId) => {
  return Object.entries(activeLayersByEntity).reduce((acc, [entityType, activeLayers]) => {
    const layerId = activeLayers.countryDefaultLayerList[countryCode?.toLowerCase()] ?? globalLayerId;
    acc[entityType as EntityType] = activeLayers.list[layerId ?? ""]?.activeCountries?.includes?.(countryCode?.toLowerCase()) ? layerId : null;
    return acc;
  }, {} as EntityStoreMap<number | null>);
})

export const $currentDefaultLayerId = combine($currentDefaultLayerIdByEntity, $selectedEntityType, $globalLayerId, (currentDefaultLayerIdByEntity, selectedEntityType, globalLayerId) => {
  return getEntityValue(currentDefaultLayerIdByEntity, selectedEntityType, globalLayerId);
})

export const $isActiveCurrentLayerByEntity = combine($activeLayerByCountriesByEntity, $selectedLayerIdByEntity, $countryCode, $selectedLayerId, (activeLayersByEntity, selectedLayerIdByEntity, countryCode, selectedLayerId) => {
  return Object.entries(activeLayersByEntity).reduce((acc, [entityType, activeLayers]) => {
    const entityLayerId = getSelectedEntityLayerId(selectedLayerIdByEntity, entityType as EntityType, selectedLayerId);
    acc[entityType as EntityType] = !!entityLayerId && activeLayers.list[entityLayerId]?.activeCountries?.includes(countryCode.toLowerCase());
    return acc;
  }, {} as EntityStoreMap<boolean>);
})

export const $isActiveCurrentLayer = combine($isActiveCurrentLayerByEntity, $selectedEntityType, (isActiveCurrentLayerByEntity, selectedEntityType) => {
  return getEntityValue(isActiveCurrentLayerByEntity, selectedEntityType, false);
})

export const $activeLayerByCountryCodeByEntity = combine($layersList, $activeLayerByCountriesByEntity, $countryCode, $activeEntityTypes, (layers, activeLayersByEntity, countryCode, activeEntityTypes) => {
  const result = {} as EntityStoreMap<Record<string, boolean>>;
  layers?.forEach((layer) => {
    getLayerEntityTypes(layer, activeEntityTypes).forEach((entityType) => {
      const activeLayers = getEntityValue(activeLayersByEntity, entityType, { list: {}, countryDefaultLayerList: {} });
      result[entityType] = {
        ...(result[entityType] ?? {}),
        [layer.id]: activeLayers.list[layer.id]?.activeCountries?.includes(countryCode.toLowerCase()) ?? false,
      };
    });
  })
  return result;
...
export const $activeLayerByCountryCode = combine($activeLayerByCountryCodeByEntity, $selectedEntityType, (activeLayerByCountryCodeByEntity, selectedEntityType) => {
  return getEntityValue(activeLayerByCountryCodeByEntity, selectedEntityType, {});
});
...
export const $selectedLayerDataByEntity = combine($layersList, $selectedLayerIdByEntity, $currentDefaultLayerIdByEntity, $selectedLayerId, (layers, selectedLayerIdByEntity, currentDefaultLayerIdByEntity, selectedLayerId) => {
  ...
  return Array.from(entityTypes).reduce((acc, entityType) => {
    const entityLayerId = getEntityValue(selectedLayerIdByEntity, entityType, currentDefaultLayerIdByEntity[entityType] ?? selectedLayerId);
    acc[entityType] = layers?.find(item => item.id === entityLayerId) ?? null;
    return acc;
...
export const $currentLayerCountryDataSource = combine($selectedLayerData, $country, (selectedData, country) => {
  ...
  return (selectedData?.active_countries_list?.find(activeLayers => activeLayers.country === country.id) as { data_sources?: unknown } | undefined)?.data_sources || null
})
...
export const $currentLayerTypeUtilsByEntity = combine(
  ...
  (statusLayerIdByEntity, selectedLayerDataByEntity) => {
    ...
    return Array.from(entityTypes).reduce((acc, entityType) => {
      ...
      return acc;
    }, {} as EntityStoreMap<{ isLive: boolean; isStatic: boolean; isSchoolStatus: boolean }>);
  })

export const $currentLayerTypeUtils = combine(
  $currentLayerTypeUtilsByEntity,
  $selectedEntityType,
  (currentLayerTypeUtilsByEntity, selectedEntityType) => getEntityValue(currentLayerTypeUtilsByEntity, selectedEntityType, {
    isLive: false,
    isStatic: false,
...
const buildCurrentLayerLegends = ({
  ...
  currentLayerTypeUtils?: { isLive: boolean; isStatic: boolean; isSchoolStatus: boolean };
  selectedLayerData?: LayerType | null;
  stylePaintData: typeof $stylePaintData extends { getState: () => infer T } ? T : never;
}) => {
  let apiLegends = selectedLayerData?.legend_configs;
  if (connectivityBenchmark === ConnectivityBenchMarks.national) {
    apiLegends = countryActiveLayersDataById[selectedLayerData?.id ?? ""]?.legend_configs
...
export const $currentLayerLegendsByEntity = combine({
  ...
  countryActiveLayersDataById: $countryActiveLayersDataById,
  connectivityBenchmark: $connectivityBenchMark,
}, ({ selectedLayerDataByEntity, currentLayerTypeUtilsByEntity, stylePaintData, connectivityBenchmark, countryActiveLayersDataById }) => {
  return Object.entries(selectedLayerDataByEntity).reduce((acc, [entityType, selectedLayerData]) => {
    acc[entityType as EntityType] = buildCurrentLayerLegends({
      selectedLayerData,
      currentLayerTypeUtils: currentLayerTypeUtilsByEntity[entityType as EntityType],
      stylePaintData,
      connectivityBenchmark,
      countryActiveLayersDataById,
    });
...
export const $benchmarkmarkUtils = combine($countryBenchmark, $selectedLayerData, $connectivityBenchMark, $countryConnectivityNames, (countryBenchmark, selectedLayerData, connectivityBenchMark, countryConnectivityNames) => {
  return buildBenchmarkUtils(countryBenchmark, selectedLayerData, connectivityBenchMark, countryConnectivityNames);
});
...
export const $benchmarkmarkUtilsByEntity = combine($countryBenchmark, $selectedLayerDataByEntity, $connectivityBenchMark, $countryConnectivityNames, (countryBenchmark, selectedLayerDataByEntity, connectivityBenchMark, countryConnectivityNames) => {
  return Object.entries(selectedLayerDataByEntity).reduce((acc, [entityType, selectedLayerData]) => {
    acc[entityType as EntityType] = buildBenchmarkUtils(countryBenchmark, selectedLayerData, connectivityBenchMark, countryConnectivityNames);
    return acc;
  }, {} as EntityStoreMap<ReturnType<typeof buildBenchmarkUtils>>);
});

export const $isNationalBenchmarkByEntity = $benchmarkmarkUtilsByEntity.map((benchmarkmarkUtilsByEntity) => {
  return Object.entries(benchmarkmarkUtilsByEntity).reduce((acc, [entityType, benchmarkmarkUtils]) => {
...
export const $staticPopupActiveLayerByEntity = combine($activeLayerByCountryCodeByEntity, $staticLayers, $coverageLayerData, $activeEntityTypes, (activeLayerByCountryCodeByEntity, staticLayers, coverageDynamicLayerData, activeEntityTypes) => {
  return activeEntityTypes.reduce((acc, entityType) => {
    const activeLayerByCountryCode = getEntityValue(activeLayerByCountryCodeByEntity, entityType, {});
    const entityStaticLayers = staticLayers.filter((layer) => getLayerEntityTypes(layer, activeEntityTypes).includes(entityType));
    if (activeLayerByCountryCode[coverageDynamicLayerData?.id ?? ""]) {
      acc[entityType] = coverageDynamicLayerData;
      return acc;
    }
    acc[entityType] = entityStaticLayers.find(item => activeLayerByCountryCode[item?.id ?? ""]) ?? null;
    return acc;
  }, {} as EntityStoreMap<LayerType | null>);
})
...
export const $isSchoolBenchmarkByEntity = combine($selectedLayerDataByEntity, $connectivityBenchMark, $country, (selectedLayerDataByEntity, conntectivityBenchmark, country) => {
  return Object.entries(selectedLayerDataByEntity).reduce((acc, [entityType, selectedLayer]) => {
    const isLive = isLiveLayer(selectedLayer?.type);
    if (!isLive) {
    ...
    if (conntectivityBenchmark === ConnectivityBenchMarks.global) {
      acc[entityType as EntityType] = !!selectedLayer?.global_benchmark?.value?.startsWith('SQL:');
    } else if (conntectivityBenchmark === ConnectivityBenchMarks.national) {
      acc[entityType as EntityType] = !!country?.benchmark_metadata?.live_layer?.[selectedLayer?.id ?? ""]?.startsWith('SQL:');
    }
    return acc;
  }, {} as EntityStoreMap<boolean>);
})
...
export const entityStaticLegendsSelection = createEvent<{ entityType: EntityType; legends: string | string[] }>();
export const selectAllStaticLegendsSelection = createEvent<string[]>();
export const selectAllEntityStaticLegendsSelection = createEvent<{ entityType: EntityType; legends?: string[] }>();
export const makeEmptyStaticLegendsSelection = createEvent<string[]>();
export const makeEmptyEntityStaticLegendsSelection = createEvent<{ entityType: EntityType }>();

const updateStaticLegendsSelection = (state: string[], payload: string | string[]) => {
...
export const $staticLegendsSelectedByEntity = createStore<EntityStoreMap<string[]>>({
...
$staticLegendsSelectedByEntity.on(staticLegendsSelection, (state, payload) => {
  ...
  return {
    ...state,
    [selectedEntityType]: updateStaticLegendsSelection(getEntityValue(state, selectedEntityType, defaultStatusLegendsSelection), payload),
  };
})
$staticLegendsSelectedByEntity.on(entityStaticLegendsSelection, (state, { entityType, legends }) => ({
  ...state,
  [entityType]: updateStaticLegendsSelection(getEntityValue(state, entityType, defaultStatusLegendsSelection), legends),
}))
...
$staticLegendsSelectedByEntity.on(makeEmptyEntityStaticLegendsSelection, (state, { entityType }) => ({
  ...state,
  [entityType]: [],
}))
$staticLegendsSelectedByEntity.on(selectAllStaticLegendsSelection, (state) => {
  const selectedEntityType = $selectedEntityType.getState();
  const currentState = getEntityValue(state, selectedEntityType, defaultStatusLegendsSelection);
...
$staticLegendsSelectedByEntity.on(selectAllEntityStaticLegendsSelection, (state, { entityType, legends }) => {
  const currentState = getEntityValue(state, entityType, defaultStatusLegendsSelection);
  if (currentState.length === 3) {
    return state;
  }
  return {
...
export const $staticLegendsSelected = combine($staticLegendsSelectedByEntity, $selectedEntityType, (staticLegendsSelectedByEntity, selectedEntityType) => {
  return getEntityValue(staticLegendsSelectedByEntity, selectedEntityType, defaultStatusLegendsSelection);
})
...
export const $coverageStatusAllByEntity = combine($coverageStatusAll, $activeEntityTypes, (coverageStatusAll, activeEntityTypes) => {
  return (activeEntityTypes.length ? activeEntityTypes : [EntityType.SCHOOL]).reduce((acc, entityType) => {
...
export const $multiSelectionSchoolCheckboxByEntity = combine($multiSelectionSchoolCheckbox, (multiSelectionSchoolCheckbox) => ({
...
export const $connectivityColorsWithBenchmarkByEntity = combine($connectivityColorsWithBenchmark, $activeEntityTypes, (connectivityColorsWithBenchmark, activeEntityTypes) => {
  return (activeEntityTypes.length ? activeEntityTypes : [EntityType.SCHOOL]).reduce((acc, entityType) => {
...
export const $connectivityAvailabilityByEntity = createStore<Partial<Record<EntityType, ConnectivityConfig>>>({});
...
$connectivityAvailabilityByEntity.on(getEntitiesAvailableDates.doneData, (_, payload) => {
  return payload as Partial<Record<EntityType, ConnectivityConfig>>;
});
$connectivityAvailability.on(getEntitiesAvailableDates.doneData, (_, payload) => {
  return (payload as Partial<Record<EntityType, ConnectivityConfig>>)?.[$selectedEntityType.getState()] ?? null;
});
...
export const $allLoadings = combine({
  ...
  lastAvailableDates: combine(getSchoolAvailableDates.pending, getEntitiesAvailableDates.pending, (schoolPending, entityPending) => schoolPending || entityPending),
})
...
export const $isLoadingCountryAdminView = $allLoadings.map(({ country, lastAvailableDates, stats, info, entityInfo, layers }) => [info, entityInfo, lastAvailableDates, country, stats, layers].some(Boolean));
...
$connectivityAvailabilityByEntity.reset(router.historyUpdated, $selectedLayerId);
