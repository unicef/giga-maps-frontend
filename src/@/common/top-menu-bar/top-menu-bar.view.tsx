  ...
  return (
    ...
    >
      <div className="flex h-11 w-full items-center justify-start gap-2 px-3.5! py-0">
        ...
        <Link
          className="flex h-4 items-center no-underline outline-none shadow-none visited:no-underline hover:no-underline focus:no-underline active:no-underline"
          to={mapOverview}
          aria-label="Giga Maps"
        >
          <GigaMapsLogo className="h-3.5! w-auto! max-w-[7.5rem]! [&_path]:fill-[#f4f4f4]!" />
        </Link>
      </div>
    </div>
  )
