...
const getActiveYears = ({
  activeEntityTypes,
  timeplayerInfo,
}: ReturnType<typeof timePlayerData.getState>) => {
  if (activeEntityTypes.length !== 1) return null;
  return timeplayerInfo.yearsByEntity[activeEntityTypes[0]] ?? null;
};

export const timePlayerSourceFx = createEffect(
  async ({ map, url }: { map: Map; url: string }) => {
...
export const timePlayerFx = createEffect(
  (data: ReturnType<typeof timePlayerData.getState>) => {
    const { map } = data;
    const years = getActiveYears(data);
    if (!map || !years) return;
...
export const onLoadStartTimePlayer = createEffect(
  (data: ReturnType<typeof timePlayerData.getState>) => {
    const { map, paintData } = data;
    const years = getActiveYears(data);
    if (!map || !years) return;
    ...
    map.setPaintProperty(
      timePlayerLayerId,
      'circle-color',
      getCirclesPaint(String(activeYear), paintData),
    );
...
export const nextTimePlayerIteration = createEffect(
  (data: ReturnType<typeof timePlayerData.getState>) => {
    const { map, paintData, timeplayerInfo } = data;
    const { activeYear: prevsYear } = timeplayerInfo;
    const years = getActiveYears(data);
    if (!map || !years) return;
    ...
    if (nextYear) {
      ...
      map.setPaintProperty(
        timePlayerLayerId,
        'circle-color',
        getCirclesPaint(String(nextYear), paintData),
      );
      runIntervalCheck();
    } else {
      // off time player
...
export const clearTimeplayer = createEffect(
  ({ map }: ReturnType<typeof timePlayerData.getState>) => {
    if (!map) return;
    map.off('data', mapDataTilesOnLoad);
    clearTimeout(timeout);
